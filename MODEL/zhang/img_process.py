...
from MODEL.zhang.data_enhance import dilated, eroded, img_bright_mean, balance_light#
import numpy as np
import math
from MODEL.zhang.aug import aug
...
# 亮度方差
P1_VUE_RANGE = (120, 140)
P2_VUE_RANGE = (135, 155)


# model1 = joblib.load('./p1.model')
# model2 = joblib.load('./p2.model')
    ...
    if len(new_cnts) > 1:
        ...
        for ncnt in new_cnts:
            ...
            if y0 > y and x > 240 and x < 300:
                ...
                y0 = y
        new_cnts = cnt0
    cv2.drawContours(img, new_cnts, -1, (0, 0, 255), 2)
...
def img_process_p1(img, type, anchor_loc, outputsize=(30, 30)):
    ...
    imgmean = img_bright_mean(hsv_img[:, :, 2])
    # print(imgmean)
    # print(imgstd)
    v_channel = balance_light(hsv_img[:, :, 2], imgmean, P1_VUE_RANGE, alpha=0.4)
    hsv_img[:, :, 2] = v_channel
    bgr_brighter = cv2.cvtColor(hsv_img, cv2.COLOR_HSV2BGR)
    # cv2.imshow('bgr_brighter',bgr_brighter)
    gray = cv2.cvtColor(bgr_brighter, cv2.COLOR_BGR2GRAY)
...
def data_augment():
    srcimgpath = r'E:\C3407841\task\AWFeedbar\datasets\image\NG\BoScrew\H4HDP3Y9Q07Y_20-11-09_17-57-55.jpg'
    save_dir = r'E:\C3407841\task\AWFeedbar\datasets\image\p1_1\ng'
    luosi_location_2 = (219, 285, 50, 50)
    ...
    def save_img(img):
        ...
        if result_size is not None:
            ...
            cv2.imwrite(os.path.join(save_dir, filename + '-' + str(count) + suffix), result_img)

    x_rang = int((width - kernel[0]) / stride[0] + 1)
    y_rang = int((height - kernel[1]) / stride[1] + 1)
    count = 1
    print(x_rang, y_rang)
    for y in range(y_rang):
        for x in range(x_rang):
            curx = kernel[0]
            print(cury - kernel[1], cury, curx - kernel[0], curx)
...
def img_process_p2(img, type, anchor_loc, outputsize=(50, 50)):
    ...
    imgmean = img_bright_mean(hsv_img[:, :, 2])
    # print(imgmean)
    v_channel = balance_light(hsv_img[:, :, 2], imgmean, P2_VUE_RANGE, alpha=0.4)
    hsv_img[:, :, 2] = v_channel
    bgr_brighter = cv2.cvtColor(hsv_img, cv2.COLOR_HSV2BGR)
    # cv2.imshow('bgr_brighter',bgr_brighter)
    gray = cv2.cvtColor(bgr_brighter, cv2.COLOR_BGR2GRAY)
    ...
    if circles is not None:
        ...
        for circle in circles[0, :]:
            ...
            #     continue
            # print(int(cent_y-radius),int(cent_y+radius),int(cent_x-radius)int(cent_x+radius))
    ...
    for root, dirs, files in os.walk(imgdir):
        for file in files:
            if file.endswith('.jpg'):
                ...
                obj_img = cv2.imread(filepath)
                root_dir, dirname = os.path.split(root)

                anchor_loc = find_anchor_point(obj_img, 0, 0)
                if anchor_loc is None:
                    anchor_loc = (0, 0)
                obj_img = img_process_p1(obj_img, dirname, anchor_loc)
                if obj_img is None:
                    print(filepath)
                    continue
...
def val(img, model_path, location, type):
    ...
    if location == 1:
        obj_img = img_process_p1(img, type, anchor_loc)
        if obj_img is None: return -1
        result = predict(obj_img, model1)
    elif location == 2:
        obj_img = img_process_p2(img, type, anchor_loc)
        if obj_img is None: return -1
...
if __name__ == "__main__":
    # imgdir =r'E:\C3407841\task\AWFeedbar\datasets\image\src'
    # save_dir =r'E:\C3407841\task\AWFeedbar\datasets\image\p1_1'
    # # model_path=r'./p1_v2.model'
    # datasets,imgnames = batch_process(imgdir,save_dir)
    ...
    # imgpath = r'E:\C3407841\task\AWFeedbar\datasets\image\src\140S\H4HDP5DBQ07V_20-11-10_17-21-44.jpg'
    # model_path = r'./p1.model'
    # #imgpath = r'E:\C3407841\task\AWFeedbar\datasets\train\ng\p120201020-161552-604.jpg'    
    # img = cv2.imread(imgpath)
    # result1 = val(img,model_path,1,'S')
    # print(result1)
    # cv2.waitKey()

    # model_path1 = r'./p1.model'
    # model_path2 = r'./p2.model'
    # imgdir = r'E:\C3407841\task\AWFeedbar\test\1204_testing_result\OK'
    ...
    #             img = cv2.imread(imgpath)
    #             result1 = val(img,model_path1,1,'S')
    #             result2 = val(img,model_path2,2,'S')
    #             print(imgpath,result1,result2)
    #             cv2.waitKey()  
    #             if result1 == -1 or result2 == -1:
    #                 cv2.waitKey()  

    # imgpath = r'E:\C3407841\task\AWFeedbar\test\2020-11-27-testing-result\NG\H4HDRLM9Q07Y_20-11-27_14-18-02.jpg'#r'E:\C3407841\task\AWFeedbar\Feedbar_10.26\140b\20201026-150901-744.jpg'
